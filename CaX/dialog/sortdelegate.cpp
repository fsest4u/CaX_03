...
void SortDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
	...
	QIcon icon = qvariant_cast<QIcon>(index.data(SORT_ROLE_ICON));
	QString title = qvariant_cast<QString>(index.data(SORT_ROLE_TITLE));
...
QSize SortDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
	QIcon icon = qvariant_cast<QIcon>(index.data(SORT_ROLE_ICON));
	QSize iconSize = icon.actualSize(option.decorationSize);
	QFont font = QApplication::font();
